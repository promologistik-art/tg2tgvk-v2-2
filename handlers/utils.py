...
async def get_current_project(telegram_id: int, context: ContextTypes.DEFAULT_TYPE) -> Project:
    ...
    async with AsyncSessionLocal() as session:
        if project_id:
            ...
            if project:
                return project
        result = await session.execute(select(Project).where(Project.user_id == telegram_id, Project.is_active == True).order_by(Project.id))
...
async def get_project_target(project_id: int) -> TargetChannel:
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(TargetChannel).where(TargetChannel.project_id == project_id, TargetChannel.is_active == True))
