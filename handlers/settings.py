...
async def set_signature_start_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"   <code>Мой канал | https://vk.com/club123456</code>\n\n"
        f"🔗 <b>Текст со ссылкой:</b>\n"
        f"   <code>Подпишись https://vk.com/moygruppa</code>\n\n"
    ...
    if query.data == "starttime_skip":
        async with AsyncSessionLocal() as session:
            await session.execute(
                sql_update(Project)
                .where(Project.id == project_id)
                .values(post_interval_hours=minutes)
            )
            await session.commit()
...
def parse_signature_input(text: str) -> str:
    """Парсит подпись. Поддерживает текст, ссылки VK и Telegram."""
    text = text.strip()
    ...
    # Оставляем ссылки как есть — VK сам сделает их кликабельными
    return text


def get_display_text(html_text: str) -> str:
    """Упрощённый метод — для VK подпись в plain text."""
    if not html_text:
        return ""
